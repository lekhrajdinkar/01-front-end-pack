...
const first_non_Nullish_value =
  null ?? undefined ?? "str" ?? 0 ?? " " ?? 23 ?? null;
console.log(
  ">>> Short circuit, first_non_Nullish_value >>>",
  first_non_Nullish_value
);
...
function printSeq(...t) {
  console.log(t);
} // REST, condence in method definition
function printSeqItemSepByLine(...t) {
  for (let i = 0; i < t.length; i++) console.log(t[i]);
}
function printSeqItemSepBySpace(...t) {
  console.log(...t);
}
...
function print_card(title, picture, action, ...detail) {
  // detail, action --> would be condesed as array
  console.log("\n\n*****************************************");
...
print_card(
  ...
  "c.png",
  ["ok", "cancel"],
  "linjcncj kvnxkcn  vnxkvn",
  "jbvjs bvjkbjv jbjxvxbj bjsbvjsd",
  "bjdsbvj dsjcjdsc sdskhjd sknvsd "
);
